function [u]=mgv(u0,rhs,N,nu1,nu2,level,max_level,k)
%
...
%
if (level == max_level)
[u,resvec,iter] = my_cg(u0,rhs,N,1.0e-8,1000,k);
else
[u , rh] = jacobi(u0,rhs,2/3,N,nu1,k);
r2h = restriction(rh,N);
e2h = mgv(zeros(N/2+1),r2h,N/2,nu1,nu2,level+1,max_level,k);
%Interpolating on finer grid
 eh = interpolation(e2h,N);
 u = u + eh;
 u = jacobi(u,rhs,2/3,N,nu2,k); 
end